	...
	export default {
		data() {
			return {
				...
				_recorderEventsBound: false,
				_scrollTimer: null,
				_focusGuardTimer: null,
				_blockInputFocus: false,
				_audioContext: null,
				_playingIndex: -1,
				_keyboardChangeHandler: null,
		...
		onLoad() {
			...
			this._recorderEventsBound = false;
			this._scrollTimer = null;
			this._focusGuardTimer = null;
			this._blockInputFocus = false;

			this._keyboardChangeHandler = ({
				height
			}) => {
			...
			this.$nextTick(() => this._updateListHeight(() => this.scrollToBottom()));
		},

		onShow() {
			this.$nextTick(() => {
				this._updateListHeight(() => this.scrollToBottom());
			});
		},
		...
		methods: {
			...
			_updateListHeight(cb) {
				...
				query.exec((res) => {
					...
					const kbH = this._keyboardHeight || 0;
					const newHeight = Math.max(0, this._windowHeight - navH - bottomH - kbH);
			...
			resetBottomState() {
				...
				this.stopCurrentAudio();
				this.$nextTick(() => this._updateListHeight());
			},
			stopCurrentAudio() {
			...
			dismissKeyboard() {
				this._blockInputFocus = true;
				this.inputFocus = false;
				clearTimeout(this._focusGuardTimer);
				uni.hideKeyboard();
				this._focusGuardTimer = setTimeout(() => {
					this._blockInputFocus = false;
				}, 200);
			},
			...
			ensureRecorderManager() {
				...
				this._recorderManager.onStop((res) => {
					...
					if (this._recordCancelled) return;
			...
			stopRecord() {
				if (!this.isRecording) return;
				this.isRecording = false;
				this._recorderManager && this._recorderManager.stop();
			},
			cancelRecord() {
				if (!this.isRecording) return;
