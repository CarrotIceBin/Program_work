...
const IMAGE_STORAGE_KEY = 'xiangQin_images';
const MAX_IMAGES = 6;
...
export default {
	...
	computed: {
		xiangQinFields() {
			...
			const predefined = this.xiangQinFieldList.map(field => {
				...
				if (field.forceText) {
					const localKey = `xiangQin_${field.key}`;
	...
	methods: {
		...
		navigateToUserMeansByField(field) {
			...
			if (!item && field.forceText) {
				infoTxt = uni.getStorageSync(`xiangQin_${field.key}`) || '';
				item = {
					moduleID: field.key,
				...
				infoTxt = uni.getStorageSync(`xiangQin_${field.key}`) || item.infoTxt;
			...
			const data = encodeURIComponent(JSON.stringify({
				...
				localKey: field.forceText ? `xiangQin_${field.key}` : ''
			}));
